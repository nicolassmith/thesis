...
% there are some set of eigenmodes of cavities, where after a complete round trip, the amplitude of the mode is maintained, perhaps with a scaling due to losses, and with some amount of total phase shift. covered in seigman

% For cavities with spherical mirrors, these modes are nearly the TEM modes of free space propagation, with a particular waist size

% a given TEM mode characterized by a waist size experienced a phase shift, in addition to the normal phase shift of a plane wave. For the TEMmn mode in the HG basis, this shift is delta \eta(1+m+n) where eta is defined as, (in terms of waist position).

% A critically coupled cavity has the property that when light resonates, it is maximally transmitted. When it is far from resonance (outside the linewidth of the cavity), it is maximally reflected.

% The total gouy phase in one round trip is cos eta_c=g? Resonance of the TEM00 mode occurs when the total round trip phase of that mode (kp+eta_c) is equal to 2pi. For TEMmn resonance, the plane wave phase must shift by an amount eta_c(m+n) relative to the TEM00 resonance. So delta L = blah or delta f = blah. Coincident resonances may occur when delta f = j*FSR.

% a mode cleaner is designed so that the desired mode (usually TEM00) can be held on resonance, while the undesirable modes are far from resonance. In most cases the amplitude of the HOMs is decreasing with increasing order. There will always be coincident resoances for sufficiently high order modes, one must design the mode cleaner such that the modes that do have coincident resonance are of sufficiently small amplitude that they don't cause problems.
