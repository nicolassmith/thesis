...
\end{equation}
where $U_m(x,z)$ is the $m$th 1-D Hermite-Gaussian mode (see \ref{ap:HGmode}). %NL%
The basis modes are also referred to as the Transverse Electric and Magnetic modes of order $mn$ (\TEM{mn}). %NL%
...
Thus one can see that when these matrix elements have a phase close to an integer multiple of $2\pi$, the cavity field of those component will be enhanced. %NL%
...
\com{maybe just a segue to the next chap?}